...
void draw_bbox(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch) {
    for (size_t i = 0; i < img_batch.size(); i++) {
        ...
        for (size_t j = 0; j < res.size(); j++) {
            ...
            std::string conf = cv::format("%.2f", res[j].conf);
            cv::putText(img, std::to_string((int)res[j].class_id) + " " + conf, cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2,
...
void draw_mask_bbox(cv::Mat& img, std::vector<Detection>& dets, std::vector<cv::Mat>& masks,
    ...
    for (size_t i = 0; i < dets.size(); i++) {
        cv::Mat img_mask = scale_mask(masks[i], img);    // recover mask size to img
	    ...
	    // bbox
        cv::rectangle(img, r, bgr, 1, cv::LINE_AA);

        // Get the size of the text
        cv::Size textSize = cv::getTextSize(labels_map[(int)dets[i].class_id] + " " + to_string_with_precision(dets[i].conf),
                                cv::FONT_HERSHEY_PLAIN, 1, 2, NULL);

        // Draw the rectangle on the image
        cv::rectangle(img, cv::Point(r.x, r.y - textSize.height - 2), cv::Point(r.x + textSize.width, r.y - 1), bgr, -1, cv::LINE_AA);
        cv::putText(img, labels_map[(int)dets[i].class_id] + " " + to_string_with_precision(dets[i].conf),
                    cv::Point(r.x, r.y - 2), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar::all(0xFF), 2, cv::LINE_AA);
    }
}
...
std::vector<cv::Point> get_corner(cv::Mat& img, const Detection& box) {
...
void draw_bbox_obb(std::vector<cv::Mat>& img_batch, std::vector<std::vector<Detection>>& res_batch) {
    ...
    for (size_t i = 0; i < img_batch.size(); i++) {
        ...
        for (auto& obj : res) {
            ...
            cv::polylines(img, std::vector<std::vector<cv::Point>>{corner_points}, true, bgr, 3);

            auto text = (std::to_string((int)(obj.class_id)) + ":" + to_string_with_precision(obj.conf));
            cv::Size textsize = cv::getTextSize(text, 0, 0.75, 2, nullptr);
            ...
            cv::putText(img, text,
                    cv::Point(corner_points[0].x, (outside ? corner_points[0].y - 2 : corner_points[0].y + height + 2)), 0, 0.75, cv::Scalar::all(255), 2, cv::LINE_AA);
        }
    }
}
